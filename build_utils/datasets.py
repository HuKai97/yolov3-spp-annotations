...
from train_val_utils.utils import xyxy2xywh
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    ...
    def __init__(self,
                 ...
                 pad=0.0, rank=-1, # 单GPU训练rank=-1
                 augment=False, # 训练集设置为True(augment_hsv)，验证集设置为False
                 rect=False,  # 是否使用rectangular training 训练False 验证为True
                 mosaic=False):   # 是否使用mosaic augment 训练True/False 验证为False
...
def exif_size(img):
    ...
    try:
        rotation = dict(img._getexif().items())[orientation]  # 获取图片的旋转信息
